...
int main(int argc, char **argv)
{
    ...
    numselected = atoi(argv[2]); // k for topk origin provenance or number of groups

    W = atoi(argv[3]); // window size W for Prov4Window
	budget = atoi(argv[4]); // for budgetProvProp
	reduction = atoi(argv[5]); // for budgetProvProp
	
	// if not topk, just select first k nodes
    ...
    t = clock(); 
    if (noProvFromMem2(inter, numinter, numnodes, selected, numselected, 1)==-1)
		return -1;
	t = clock() - t;
	...
	//initialize groups
	map = (int *)malloc(numselected*sizeof(int));	
	numgroups = numselected;
	//assignment by a simple hash function (should be replaced by something else)
	for(i=0;i<numnodes;i++)
		map[i] = i % numgroups;
	t = clock(); 
	...
	// uncomment to run Window and Budget models
	/*
